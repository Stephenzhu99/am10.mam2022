...
emperor_killers %>% 
  mutate(
    enemy = ifelse(killer == "Other Emperor" | killer == 'Opposing Army', TRUE, FALSE),
    # use `fct_inorder()` to maintain the way we sorted the data
    killer = fct_inorder(killer)
  ) %>%
  ggplot(aes(x = n, y = killer, fill = enemy)) +
  geom_col() +
...
We want to create a graph that looks at African countries where life expectancy between 1992 and 2007 went down.
